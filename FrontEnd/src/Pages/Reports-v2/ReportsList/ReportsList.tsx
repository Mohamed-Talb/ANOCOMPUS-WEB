...
function PriorityBadge({ priority }: { priority: string }) {
  return (
    <span
      className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium ${getPriorityColor(
        priority
      )}`}
    >
      {priority}
...
// ✅ Report Card Component (updated style)
function ReportCard({
  report,
  onClick,
}: {
  ...
  return (
    <div
      onClick={onClick}
      className="bg-[#1c1c1e] border border-[#2c2c2e] rounded-xl hover:bg-[#2c2c2e] transition-colors cursor-pointer p-4"
    >
      <div className="flex flex-col gap-3">
        {/* Title and Description */}
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[#98989d] text-sm truncate">
...
export default function ReportsList({
  ...
  return (
    <div className="h-full bg-transparent text-white p-4 sm:p-6 lg:p-8">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-3">
            <button className="flex items-center gap-2 px-4 py-2 bg-[#1c1c1e] border border-[#2c2c2e] rounded-lg hover:bg-[#2c2c2e] transition-colors text-sm">
              Sort <ChevronDown className="w-4 h-4" />
            </button>
